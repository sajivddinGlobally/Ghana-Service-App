...
class LeaveRequestNotifier extends StateNotifier<AsyncValue<GetMyLeaveModel>> {
  ...
  final List<Doc> _fullList = [];

  LeaveRequestNotifier(this.authService) : super(const AsyncValue.loading()) {
  ...
  Future<void> fetchAllLeave({bool isRefresh = false}) async {
    try {
      if (isRefresh) {
        _currentPage = 1;
        _totalPages = 1;
  ...
  Future<void> loadNextPage() async {
    ...
    try {
      ...
      if (response.code == 0 && response.error == false) {
        ...
        updatedModel.data?.docs = _fullList;
...
final leaveRequestProvider =
    StateNotifierProvider.autoDispose<
      ...
      AsyncValue<GetMyLeaveModel>
    >((ref) {
      final authService = ref.read(authServiceProvider);
      return LeaveRequestNotifier(authService);
    });
