...
class GetTicketNotifier
    extends StateNotifier<AsyncValue<GetTicketModel>> {
  ...
  GetTicketNotifier(this.authService) : super(const AsyncValue.loading()) {
  ...
  Future<void> getTicket({bool isRefresh = false}) async {
    try {
      if (isRefresh) {
        _currentPage = 1;
        _totalPages = 1;
      ...
      final response = await authService.getTicket(page: _currentPage);
...
final getTicketProvider =
    StateNotifierProvider.autoDispose<
      GetTicketNotifier,
      AsyncValue<GetTicketModel>
    >((ref) {
      final authService = ref.read(authServiceProvider);
      return GetTicketNotifier(authService);
    });
